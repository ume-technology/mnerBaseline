...
from nercodebasetplinker_.tplinker_plus_ner import config
import torch
from torch.utils.data import DataLoader, Dataset
import wandb
from nercodebasetplinker_.common.utils import Preprocessor, DefaultLogger
import json
from transformers import BertModel, BertTokenizerFast
from nercodebasetplinker_.tplinker_plus_ner.tplinker_plus_ner import (HandshakingTaggingScheme,
                                                                      DataMaker4Bert,
                                                                      DataMaker4BiLSTM,
                                                                      TPLinkerPlusBert,
                                                                      TPLinkerPlusBiLSTM,
...
def train_step(batch_train_data, optimizer):
    if config["encoder"] == "BERT":
        ...
        batch_input_ids, batch_attention_mask, batch_token_type_ids, batch_shaking_tag = (
        ...
        batch_input_ids, batch_shaking_tag = (batch_input_ids.to(device), batch_shaking_tag.to(device))
...
def valid_step(batch_valid_data):
    ...
    with torch.no_grad():
        if config["encoder"] == "BERT":
            pred_shaking_outputs, _ = ent_extractor(batch_input_ids, batch_attention_mask, batch_token_type_ids, )
        elif config["encoder"] in {"BiLSTM", }:
            pred_shaking_outputs, _ = ent_extractor(batch_input_ids)

    pred_shaking_tag = (pred_shaking_outputs > 0.).long()
    sample_acc = metrics.get_sample_accuracy(pred_shaking_tag, batch_shaking_tag)
    cpg_dict = metrics.get_cpg(sample_list, tok2char_span_list, pred_shaking_tag, hyper_parameters["match_pattern"])
...
def train_n_valid(train_dataloader, dev_dataloader, optimizer, scheduler, num_epoch):
    ...
    def valid(dataloader, ep):
        ...
        if "ent_cpg" in total_cpg_dict:
            ...
            trigger_iden_prf = metrics.get_prf_scores(total_cpg_dict["trigger_iden_cpg"][0], total_cpg_dict["trigger_iden_cpg"][1], total_cpg_dict["trigger_iden_cpg"][2])
            trigger_class_prf = metrics.get_prf_scores(total_cpg_dict["trigger_class_cpg"][0], total_cpg_dict["trigger_class_cpg"][1], total_cpg_dict["trigger_class_cpg"][2])
