...
class HandshakingKernel(nn.Module):
    ...
    def forward(self, seq_hiddens):
        ...
        for ind in range(seq_len):
            hidden_each_step = seq_hiddens[:, ind, :]  # seq_hiddens:(batch_size,1,hidden_size)
            visible_hiddens = seq_hiddens[:, ind:, :]  # ind: only look back #TODO:这里是向后看? visible_hiddens与repeat_hiddens形状相同?
            ...
            # shaking_hiddens_list里放的是(batch_size,seq_len,hidden_size),(batch_size,seq_len-1,hidden_size),(batch_size,seq_len-2,hidden_size)...(batch_size,1,hidden_size)
