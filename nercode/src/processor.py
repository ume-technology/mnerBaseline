...
# old tagging function
# ENTITY_TYPES = ["OBJ", "TAS", "FUNC", "BOD", "EAT", "SCE", "PRT", "SPO", "BOF", "TIME", "CON", "LOC", "MAT", "AGE"]

# new correct tagging function  todo version tagdata2ner_new
ENTITY_TYPES = [
    "mar图案",
    "sce预防保护",
    "cro身体部位",
...
# todo version tagdata2ner_20230301
ENTITY_TYPES = ['cro家具设备', 'sce睡觉休息', 'sce整理搬运', 'mar舒适触感', 'cro办公学习', 'sce运动健身', 'mar图案', 'sce身体保健', 'sce乐业', 'cro弱势群体', 'sce工作学习',
...
class CRFFeature(BaseFeature):
    def __init__(self, token_ids, attention_masks, token_type_ids, labels=None, pseudo=None, distant_labels=None):
        super(CRFFeature, self).__init__(token_ids=token_ids, attention_masks=attention_masks,
...
class SpanFeature(BaseFeature):
    def __init__(self, token_ids, attention_masks, token_type_ids, start_ids=None, end_ids=None, pseudo=None):
        super(SpanFeature, self).__init__(token_ids=token_ids, attention_masks=attention_masks,
                                          token_type_ids=token_type_ids)
...
class MRCFeature(BaseFeature):
    def __init__(self, token_ids, attention_masks, token_type_ids, ent_type=None, start_ids=None, end_ids=None,
                 pseudo=None):
        super(MRCFeature, self).__init__(token_ids=token_ids, attention_masks=attention_masks,
                                         token_type_ids=token_type_ids)
    ...
    def get_examples(self, raw_examples, set_type):
        ...
        for i, item in enumerate(raw_examples):
            ...
            for sent in sentences:
                ...
                examples.append(InputExample(set_type=set_type, text=sent, labels=labels, pseudo=pseudo,
...
def convert_crf_example(ex_idx, example: InputExample, tokenizer: BertTokenizer, max_seq_len, ent2id):
    ...
    encode_dict = tokenizer.encode_plus(text=tokens, max_length=max_seq_len, pad_to_max_length=True,
                                        is_pretokenized=True, return_token_type_ids=True, return_attention_mask=True)
...
def convert_span_example(ex_idx, example: InputExample, tokenizer: BertTokenizer, max_seq_len, ent2id):
    ...
    feature = SpanFeature(token_ids=token_ids, attention_masks=attention_masks, token_type_ids=token_type_ids,
                          start_ids=start_ids, end_ids=end_ids, pseudo=pseudo)
    return feature, callback_info


def convert_mrc_example(ex_idx, example: InputExample, tokenizer: BertTokenizer, max_seq_len, ent2id, ent2query,
                        mask_prob=None):
    set_type = example.set_type
    text_b = example.text
    entities = example.labels
    ...
    if set_type == 'train':
        ...
        for _type in ENTITY_TYPES:
            ...
            encode_dict = tokenizer.encode_plus(
                ...
                truncation_strategy='only_second', is_pretokenized=True, return_token_type_ids=True,
            ...
            feature = MRCFeature(token_ids=token_ids, attention_masks=attention_masks,
                                 token_type_ids=token_type_ids, ent_type=ent2id[_type], start_ids=start_ids,
                                 end_ids=end_ids, pseudo=pseudo)
    ...
    # 测试数据构造，为每一类单独构造一个 example
        ...
        for _type in ENTITY_TYPES:
            text_a = ent2query[_type]
            tokens_a = fine_grade_tokenize(text_a, tokenizer)
            encode_dict = tokenizer.encode_plus(text=tokens_a, text_pair=tokens_b, max_length=max_seq_len,
                                                pad_to_max_length=True,
                                                truncation_strategy='only_second', is_pretokenized=True,
            ...
            feature = MRCFeature(token_ids=token_ids, attention_masks=attention_masks, token_type_ids=token_type_ids,
...
def convert_examples_to_features(task_type, examples, max_seq_len, bert_dir, ent2id):
    ...
    for i, example in enumerate(examples):
        if task_type == 'crf':
            feature, tmp_callback = convert_crf_example(ex_idx=i, example=example, max_seq_len=max_seq_len,
                                                        ent2id=ent2id, tokenizer=tokenizer)
        elif task_type == 'mrc':
            feature, tmp_callback = convert_mrc_example(ex_idx=i, example=example, max_seq_len=max_seq_len,
                                                        ent2id=type2id, ent2query=ent2id, tokenizer=tokenizer)
        else:
            feature, tmp_callback = convert_span_example(ex_idx=i, example=example, max_seq_len=max_seq_len,
                                                         ent2id=ent2id, tokenizer=tokenizer)
        if feature is None:
            continue
        if task_type == 'mrc':
