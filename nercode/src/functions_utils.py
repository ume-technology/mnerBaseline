...
def load_model_and_parallel(model, gpu_ids, ckpt_path=None, strict=True):
    ...
    # device = torch.device("cpu" if gpu_ids[0] == '-1' else "cuda:" + gpu_ids[0])  # todo change windows
...
def get_model_path_list(base_dir):
    ...
    model_lists = sorted(
        model_lists,
        key=lambda x: (x.split('/')[-3], int(x.split('/')[-2].split('-')[-1]))  # todo change linux
        # key=lambda x: (x.split('\\')[-3], int(x.split('\\')[-2].split('-')[-1]))  # todo change Windows
    )
