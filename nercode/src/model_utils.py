...
from nercode.src.functions_utils import vote
...
# from itertools import repeat
# from transformers import BertModel
# from ..src.functions_utils import vote
    ...
    def forward(self, token_ids, attention_masks, token_type_ids, labels=None, pseudo=None):
        ...
        if labels is not None:
            if pseudo is not None:
                # (batch,)
                tokens_loss = -1. * self.crf_module(emissions=emissions, tags=labels.long(), mask=attention_masks.byte(), reduction='none')
                # nums of pseudo datatc
                ...
                tokens_loss = -1. * self.crf_module(emissions=emissions, tags=labels.long(), mask=attention_masks.byte(), reduction='mean')
...
def build_model(task_type, bert_dir, **kwargs):
    ...
    if task_type == 'crf':
        model = CRFModel(bert_dir=bert_dir, num_tags=kwargs.pop('num_tags'), dropout_prob=kwargs.pop('dropout_prob', 0.1))
    elif task_type == 'mrc':
        model = MRCModel(bert_dir=bert_dir, dropout_prob=kwargs.pop('dropout_prob', 0.1),
                         use_type_embed=kwargs.pop('use_type_embed'), loss_type=kwargs.pop('loss_type', 'ce'))

    else:
        model = SpanModel(bert_dir=bert_dir, num_tags=kwargs.pop('num_tags'),
                          dropout_prob=kwargs.pop('dropout_prob', 0.1), loss_type=kwargs.pop('loss_type', 'ce'))
