...
from nercode.src.attack_train_utils import FGM, PGD
from nercode.src.functions_utils import load_model_and_parallel, swa
...
# from torch.utils.data import DataLoader, RandomSampler
# from transformers import AdamW, get_linear_schedule_with_warmup
# from ..src.attack_train_utils import FGM, PGD
...
def build_optimizer_and_scheduler(opt, model, t_total):
    ...
    optimizer_grouped_parameters = [
        # bert other module
        {"params": [p for n, p in bert_param_optimizer if not any(nd in n for nd in no_decay)], "weight_decay": opt.weight_decay, 'lr': opt.lr},
        {"params": [p for n, p in bert_param_optimizer if any(nd in n for nd in no_decay)], "weight_decay": 0.0, 'lr': opt.lr},
        # 其他模块，差分学习率
        {"params": [p for n, p in other_param_optimizer if not any(nd in n for nd in no_decay)], "weight_decay": opt.weight_decay, 'lr': opt.other_lr},
        {"params": [p for n, p in other_param_optimizer if any(nd in n for nd in no_decay)], "weight_decay": 0.0, 'lr': opt.other_lr},
    ]

    optimizer = AdamW(optimizer_grouped_parameters, lr=opt.lr, eps=opt.adam_epsilon)
    scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=int(opt.warmup_proportion * t_total), num_training_steps=t_total)

    return optimizer, scheduler


def train(opt, model, train_dataset):
    swa_raw_model = copy.deepcopy(model)  # copy original model
    train_sampler = RandomSampler(train_dataset)
    train_loader = DataLoader(dataset=train_dataset, batch_size=opt.train_batch_size, sampler=train_sampler, num_workers=0)
    scaler = None
    if opt.use_fp16:
