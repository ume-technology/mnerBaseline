...
"""
@Author:UmeAI
@File:competition_predict.py
@Time:2022/12/9 12:25
@Read: 
"""
# windows
import os
...
from nercode.src.model_utils import CRFModel, SpanModel, EnsembleCRFModel, EnsembleSpanModel
from nercode.src.evaluator import crf_decode, span_decode
from nercode.src.functions_utils import load_model_and_parallel, ensemble_vote
...
CKPT_PATH = r'F:\allinoneisfast\giikinnerpart-0\outgiikinner\roberta_wwm_crf\checkpoint-533\model.pt'

# ensemble_predict
BERT_DIR_LIST = [
    r'F:\allinoneisfast\premodelfiles\hflchinese-roberta-wwm-ext'  # "./bert/torch_uer_large",
]
with open('./best_ckpt_path.txt', 'r', encoding='utf-8') as f:
...
def ensemble_predict():
    ...
    if TASK_TYPE == 'crf':
        model = EnsembleCRFModel(model_path_list=model_path_list, bert_dir_list=BERT_DIR_LIST,
                                 num_tags=len(info_dict['id2ent']), device=device, lamb=LAMBDA)
    else:
        model = EnsembleSpanModel(model_path_list=model_path_list,
                                  bert_dir_list=BERT_DIR_LIST, num_tags=len(info_dict['id2ent']) + 1, device=device)
