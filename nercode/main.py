# !/mnt/d/downLinuxCondaEnvs/pyt171/bin/python
# -*- coding:utf-8 _*-
"""
@Author:UmeAI
@File:main.py
...
from nercode.src.trainer import train
from nercode.src.options import Args
from nercode.src.model_utils import build_model
from nercode.src.dataset_utils import NERDataset
from nercode.src.evaluator import crf_evaluation, span_evaluation, mrc_evaluation
from nercode.src.functions_utils import set_seed, get_model_path_list, load_model_and_parallel, get_time_dif
from nercode.src.processor import NERProcessor, convert_examples_to_features
...
# from .src.options import Args
# from .src.model_utils import build_model
# from .src.dataset_utils import NERDataset
# from .src.evaluator import crf_evaluation, span_evaluation, mrc_evaluation
# from .src.functions_utils import set_seed, get_model_path_list, load_model_and_parallel, get_time_dif
# from .src.processor import NERProcessor, convert_examples_to_features
...
def train_base(opt, train_examples, dev_examples=None):
    with open(os.path.join(opt.mid_data_dir, f'{opt.task_type}_ent2id.json'), encoding='utf-8') as f:
        ent2id = json.load(f)
    out = convert_examples_to_features(opt.task_type, train_examples, opt.max_seq_len, opt.bert_dir, ent2id)
    train_features = out[0]
    train_dataset = NERDataset(opt.task_type, train_features, 'train', use_type_embed=opt.use_type_embed)
    if opt.task_type == 'crf':
        model = build_model('crf', opt.bert_dir, num_tags=len(ent2id), dropout_prob=opt.dropout_prob)
    elif opt.task_type == 'mrc':
        model = build_model('mrc', opt.bert_dir, dropout_prob=opt.dropout_prob, use_type_embed=opt.use_type_embed, loss_type=opt.loss_type)
    else:
        model = build_model('span', opt.bert_dir, num_tags=len(ent2id) + 1, dropout_prob=opt.dropout_prob, loss_type=opt.loss_type)
    # train(opt, model, train_dataset)  # todo change
    logger.info('ending - epochs ----------- and start dev process')
    if dev_examples is not None:
        dev_features, dev_callback_info = convert_examples_to_features(opt.task_type, dev_examples, opt.max_seq_len, opt.bert_dir, ent2id)
        ...
        # model_path_list = [r'F:\Pictures\g-core-mytcner\outputs\giikin_alltageddata_new\roberta_wwm_crf\checkpoint-840\model.pt']
        metric_str = ''
        max_f1 = 0.
        max_f1_step = 0
        max_f1_path = ''
        for idx, model_path in enumerate(model_path_list):
            tmp_step = model_path.split('/')[-2].split('-')[-1]  # todo change Linux
            # tmp_step = model_path.split('\\')[-2].split('-')[-1]  # todo change Windows
            model, device = load_model_and_parallel(model, opt.gpu_ids[0], ckpt_path=model_path)  # todo change Linux
            # model, device = load_model_and_parallel(model, opt.gpu_ids, ckpt_path=model_path)  # todo change Windows
        ...
        with open(eval_save_path, 'a', encoding='utf-8') as f1:
            f1.write(metric_str)
        with open(r'F:\Pictures\g-core-mytcner\bigfiles\nerCodeBaseLineOutput\giikin_allatgeddata_20230301\best_ckpt_path.txt',  # todo change
        ...
        import shutil
        del_dir_list = [
            os.path.join(opt.output_dir, path.split('/')[-2])
...
def training(opt):
    ...
    # wait 这里是Test伪标签数据的使用过程；这里的数据直接被当作Train data数据使用 + 原始数据集中的 pseudo data set
    train_raw_examples = processor.read_json(os.path.join(opt.raw_data_dir, 'train.json'))
    # add pseudo datatc to train datatc
...
def stacking(opt):
    ...
    if args.task_type == 'mrc':
        processor = NERProcessor(opt.max_seq_len - 62)  # 62 for mrc query
    else:
        processor = NERProcessor(opt.max_seq_len)
