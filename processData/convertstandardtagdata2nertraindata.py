...
def getSpanLabelCLS():
    ...
    labelcls = set()
    with open('./tagdata2ner_20230301/taged_data/20230309Concat.txt', 'r', encoding='utf-8') as f:  # 新做出的BIES标注且check后的数据，准备做TCNER数据的格式转换
        lines = f.readlines()
        for i in lines:
            i = i.strip()
            if not i:
    ...
    span_ent2id = json.dumps(SPAN_ENT2ID, ensure_ascii=False)
    with open('tagdata2ner_20230301/mid_data/span_ent2id.json', 'w', encoding='utf-8') as f:  # √：14
        f.write(span_ent2id)
    # ent_types = ['O'] + [p + '-' + _type for p in ['B', 'I', 'E', 'S'] for _type in list(SPAN_ENT2ID)]
    ent_types = ['o'] + [p + '-' + _type for p in ['b', 'i', 'e', 's'] for _type in list(SPAN_ENT2ID)]
    CRF_ENT2ID = {ent: i for i, ent in enumerate(ent_types)}
    crf_ent2id = json.dumps(CRF_ENT2ID, ensure_ascii=False)
    with open('tagdata2ner_20230301/mid_data/crf_ent2id.json', 'w', encoding='utf-8') as f:  # √：57
        f.write(crf_ent2id)


# todo 生成mid_data下标签相关文件
# getSpanLabelCLS()
...
def preprocess(input_path, save_path, mode):
    ...
    data_path = os.path.join(save_path, mode + ".json")  # ./mid_data_most_old/train.json
    labels = set()
    result = []
    tmp = {}
    tmp['id'] = 0
    ...
    with open(data_path, 'w', encoding='utf-8') as fp:  # # ./mid_data_most_old/train.json
        fp.write(json.dumps(result, ensure_ascii=False))

    # todo: 生成本次NER任务一共有多少中标签，这个标签数据可以不写；例如本次giikin任务的标签：["SCE", "LOC", "OBJ", "BOD", "TAS", "SPO", "PRT", "FUNC", "AGE", "TIME", "MAT", "CON", "BOF", "EAT"]
    if mode == "train":
...
# preprocess("tagdata2ner_20230301/taged_data/20230309Concat.txt", './mid_data', "train")  # 本质上是tc要求的 stack 文件；
bre = 'break'
# important 生成该文件之后需要执行split脚本把这个stack数据分割为 train 和 dev 数据集; 因此dev和test数据不在这里生成；
# preprocess("dev.char.bmes", '../mid_data_most_old', "dev")
# preprocess("test.char.bmes", '../mid_data_most_old', "test")


# todo 这里生成的就是CRF ent2id数据文件；已经在getSpanLabelCLS()都生成，因此这里可以不再执行
# labels_path = os.path.join('../mid_data_most_old/labels.json')
...
#     label2id[v] = k
# path = '../mid_data_most_old'
# if not os.path.exists(path):
#     os.makedirs(path)
# else:
#     # with open(os.path.join(path, "ner_ent2id.json"), 'w', encoding='utf-8') as fp:
